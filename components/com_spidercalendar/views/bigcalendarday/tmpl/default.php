...
<div class="views" style="<?php if(!in_array('day',$views) AND $defaultview!='day' ) echo 'display:none;' ;if($view=='bigcalendarday') echo 'background-color:'.$bg_top.';height:30px;top: 0' ?>" onclick="showbigcalendar( 'bigcalendar<?php echo $id ?>','<?php echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendarday&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id=&cat_ids='.$cat_ids).'&format=row&tmpl=component&Itemid='.JRequest::getVar( "Itemid","").'&date='.$year.'-'.add_0((Month_num($month))).'-'.date('d') ;?>','<?php echo $id ?>','<?php echo $border_radius2 ?>')" ><span style="position:relative;top:25%;color:<?php echo $views_tabs_text_color ?>;font-size:<?php echo $views_tabs_font_size  ?>px"><?php echo JText::_('TAB_DAY') ?></span></div>

<div class="views" style="<?php if(!in_array('week',$views) AND $defaultview!='week' ) echo 'display:none;' ;if($view=='bigcalendarweek') echo 'background-color:'.$bg_top.';height:30px;top: 0' ?>" onclick="showbigcalendar( 'bigcalendar<?php echo $id ?>','<?php echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendarweek&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id=&cat_ids='.$cat_ids).'&format=row&tmpl=component&Itemid='.JRequest::getVar( "Itemid","").'&months='.$prev_month.','.$this_month.','.$next_month.'&date='.$year.'-'.add_0((Month_num($month))).'-'.date('d') ;?>','<?php echo $id ?>','<?php echo $border_radius2 ?>')" ><span style="position:relative;top:25%;color:<?php echo $views_tabs_text_color ?>;font-size:<?php echo $views_tabs_font_size  ?>px"><?php echo JText::_('TAB_WEEK') ?></span></div>

<div class="views" style="<?php if(!in_array('list',$views) AND $defaultview!='list' ) echo 'display:none;' ; if($view=='bigcalendarlist') echo 'background-color:'.$bg_top.';height:30px;top: 0' ?>" onclick="showbigcalendar( 'bigcalendar<?php echo $id ?>','<?php echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendarlist&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id=&cat_ids='.$cat_ids).'&format=row&tmpl=component&Itemid='.JRequest::getVar( "Itemid","").'&date='.$year.'-'.add_0((Month_num($month))) ;?>','<?php echo $id ?>','<?php echo $border_radius2 ?>')" ><span style="position:relative;top:25%;color:<?php echo $views_tabs_text_color ?>;font-size:<?php echo $views_tabs_font_size  ?>px"><?php echo JText::_('TAB_LIST') ?></span></div>

<div class="views" style="<?php if(!in_array('month',$views) AND $defaultview!='month' ) echo 'display:none;' ; if($view=='bigcalendar') echo 'background-color:'.$bg_top.';height:30px;top: 0' ?>" onclick="showbigcalendar( 'bigcalendar<?php echo $id ?>','<?php echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendar&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id=&cat_ids='.$cat_ids).'&format=row&tmpl=component&Itemid='.JRequest::getVar( "Itemid","").'&date='.$year.'-'.add_0((Month_num($month))) ;?>','<?php echo $id ?>','<?php echo $border_radius2 ?>')" ><span style="position:relative;top:25%;color:<?php echo $views_tabs_text_color ?>;font-size:<?php echo $views_tabs_font_size  ?>px"><?php echo JText::_('TAB_MONTH') ?></span></div>
</div>
</td>
                   ...
                   <table cellpadding="0" cellspacing="0" border="0" align="center" class="calyear_table"  style="margin:0; padding:0; text-align:center; width:100%; height:<?php echo $top_height ?>px;">
								...
								<tr>
									...
									<a  style="text-shadow: 1px 1px 2px black;color:<?php echo $color_arrow_month ?>"
											...
											echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendarday&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id=&cat_ids='.$cat_ids).'&format=row&Itemid='.JRequest::getVar( "Itemid","").'&date='.$year.'-'.add_0((Month_num($month)-1)).'-'.$prev_month_day_count;
											else
											{
											if(Month_num(Month_name(Month_num($month)-1))=='12' && $day=='01')
											echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendarday&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id=&cat_ids='.$cat_ids).'&format=row&Itemid='.JRequest::getVar( "Itemid","").'&date='.($year-1).'-'.add_0((Month_num($month)-1)).'-'.$prev_month_day_count;
											
											else echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendarday&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id=&cat_ids='.$cat_ids).'&format=row&Itemid='.JRequest::getVar( "Itemid","").'&date='.$year.'-'.add_0((Month_num($month))).'-'.add_0($day-1);
											}
									?>','<?php echo $id ?>')">&#9668;</a>
									</td>
									...
									<a  style="text-shadow: 1px 1px 2px black;color:<?php echo $color_arrow_month ?>" href="javascript:showbigcalendar( 'bigcalendar<?php echo $id ?>','<?php  
											...
											echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendarday&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id=&cat_ids='.$cat_ids).'&format=row&Itemid='.JRequest::getVar( "Itemid","").'&date='.$year.'-'.add_0((Month_num($month)+1)).'-01';
											else
											{
											if(Month_num(Month_name(Month_num($month)+1))=='1' && $day==$month_day_count)
											echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendarday&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id=&cat_ids='.$cat_ids).'&format=row&Itemid='.JRequest::getVar( "Itemid","").'&date='.($year+1).'-'.add_0((Month_num($month)+1)).'-01';
									...
									<td width="15%">
								<div onclick="javascript:showbigcalendar( 'bigcalendar<?php echo $id ?>','<?php echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendarday&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id=&cat_ids='.$cat_ids).'&format=row&Itemid='.JRequest::getVar( "Itemid","").'&date='.($year+1).'-'.add_0((Month_num($month))).'-'.$day; ?>','<?php echo $id ?>')" style="cursor:pointer;width:100%; height:35px; background-color: rgba(0,0,0,0.3);"><span style="top: 25%;position: relative;font-size: 26px;color:<?php echo $bg_top ?>"><?php echo $year+1 ?></span></div>
								</td>
	       ...
	       $cat_color = $db->loadResult();
				...
				$table_color=$event_bg_color1;
					...
					if($cat_color!='')
						...
						echo '<table style="border-spacing:0;height:'.$event_table_height.'px;border-spacing:0;width: 100%;background-color:'.$table_color.'"><tr><td style="font-size:'.$event_num_font_size.'px;font-weight:bold;width:15px;text-align:center;background-color:'.$color.';color:'.$event_num_color.'">'.($j+1).'</td><td ><a class="modal'.$id.'"  rel="{handler: \'iframe\', size: {x: '.$popup_width.', y: '.$popup_height.'}}" style="font-size:15px;background:none;text-decoration:none;color:'.$event_title_color.'; " href="'.JRoute::_('index.php?option=com_spidercalendar&view=spidercalendarbig&theme_id='.$theme_id.'&calendar_id='.$calendar_id.'&ev_ids='.$ev_ids_inline.'&eventID='.$ev_id[$j].'&date='.$year.'-'.add_0(Month_num($month)).'-'.(int)$day).'&tmpl=component&Itemid='.$Itemid.'" ><b>'.$ev_title[$j].'</b></a></td></tr></table>';
...
<li style="float:left;"><p class="categories1" style="background-color:#<?php echo $category->color;?>">&nbsp;&nbsp;&nbsp;&nbsp;</p><p class="categories2" id="category<?php echo $category->id ?>" style="color:#<?php echo $category->color?>" onclick="showbigcalendar( 'bigcalendar<?php echo $id ?>','<?php echo JRoute::_('index.php?option=com_spidercalendar&view=bigcalendarday&def_view='.$defaultview.'&views='.$viewselect.'&rand='.$id.'&theme_id='.$theme_id.'&calendar='.$calendar_id.'&cat_id='.$category->id.'&cat_ids='.$cat_ids ).'&format=row&tmpl=component&Itemid='.JRequest::getVar( "Itemid","").'&months='.$prev_month.','.$this_month.','.$next_month.'&date='.$year.'-'.add_0((Month_num($month))).'-'.date('d') ;?>','<?php echo $id ?>')" > <?php echo  $category->title ?></p></li>
