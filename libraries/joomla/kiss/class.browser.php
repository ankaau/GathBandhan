 ...
 * @copyright			Copyright (C) 2010 - 2014 KISS Software. All rights reserved.
 *               
 */
...
class KSBrowser {
  ...
  public function detectBrowser() {
        ...
        if ($fb_crawler) {
            ...
            switch ($browser->getBrowser()) {
                ...
                case 'chrome':
                    ...
                    break;
                case 'opera':
                	if(preg_match('/opera\smobi/i', $browser->getAgentString()) || preg_match('/opera mini/i',$browser->getAgentString())) {
				...
				case '':                    
                    foreach($userAgents as $item)
                    {
                     if(preg_match('/'.$item.'/i', $browser->getAgentString())){
                        ...
                        $result->set('mobile', false);
                        $result->set('css3', true);
						$result->set('robot', true);
                        break;
                     } else {
                        $result->set('browser', 'handheld');
                        $result->set('mobile', true);
                        $result->set('css3', false);
						$result->set('robot', false);
                     } 
                    }
                	break;
            }
		...
		if ($this->mobile_device_detect()) { $result->set('mobile', true); }
        if(isset($_COOKIE['KSMobile'.JText::_('TPL_GK_APPSITE_NAME')]) && 
        	$_COOKIE['KSMobile'.JText::_('TPL_GK_APPSITE_NAME')] == 'desktop') {
        	$result->set('mobile', false);
        	$result->set('browser', 'chrome');
        }
